...
void setup(){
  ...
  gps.f_get_position(&lat, &lon, &age); // Obtengo posicion GPS
  waitForMachine(9000);
  ...
  if(scaleCalculation > 0){
    ...
    gps.crack_datetime(&y, &m, &d, &hour, &min, &seg, &hundredths, &age); // Obtengo hora del gps
	  if(y!=0)recorder.setDate(y,m,d,hour,min);
    recorder.showTime();
  ...
  pinMode(TRIG, OUTPUT);
  pinMode(ECHO, INPUT);
  Serial.print("\n[CAL]\tUltrasonido: ");Serial.println(sonar.ping_cm());
  Serial.println("\n[MSJ]\tListo para medir.");
}
